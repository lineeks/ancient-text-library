# Aether-Cycle・五术古籍图书馆

> 面向五术（山 / 医 / 命 / 相 / 卜）的结构化古籍文库：**即时检索・引经据典・原汁原味**。
>
> 定位为「**带检索功能的图书馆**」——重索引、轻搜索，元数据驱动而非全文模糊搜索。
> 每条古籍条文以 Markdown + YAML Frontmatter 标引，命部八字保留精确 conditions
> （日干 / 月令 / 格局 / 十神 / 神煞 / 日柱 / 时柱），其他四术用 category/subcategory/type/keywords
> 轻量索引，毫秒级返回 **原文 / 古注 / 白话提要** 分层内容。

截至当前，全库收录 **115 部典籍、6041 个结构化条目**，五术全覆盖，卜部三式齐备（奇门+六壬+太乙），卜部六爻六典齐备（火珠林+增删卜易+卜筮正宗+黄金策+易隐+易冒），山部丹道五典齐备（参同契+黄庭经+悟真篇+性命圭旨+钟吕传道集），山部养生四典齐备（易筋经+八段锦+五禽戏+太极），山部符咒类已扩展至11部（咒鬼经+洞渊神咒+正一法文+玄坛刊误论+北帝七元4部+道法会元精选+盟威法箓），山部武术太极拳6篇，相部人相三典齐备（神相全编+柳庄相法+麻衣神相），地相七大经典齐备（撼龙经+葬书+青囊奥语+雪心赋+博山篇+催官篇+地理人子须知），医部经典齐备（素问+灵枢+难经+伤寒论+金匮要略+神农本草经+脾胃论+兰室秘藏+丹溪心法+温病条辨+医学心悟+血证论+医林改错+温热论+湿热病篇+医学衷中参西录+医学正传+医学入门+本草纲目精选+景岳全书精选+医方集解+本草备要+小儿药证直诀+医宗金鉴精选+外台秘要精选），医部入门四典齐备（汤头歌诀+药性赋+医学三字经+濒湖脉学），针灸甲乙经97条按篇目细切，白话精译覆盖绝大多数篇目，质量门全部通过；
Release：v1.0（1547条）→ v2.0（3373条）→ v2.1（4430条）→ v2.2（4680条）→ v2.3（4808条，全库白话增强）→ v2.4（4857条，医部二书+地相二书+黄金策精译）→ v2.5（4908条，地理人子须知）→ v2.6（4913条，称骨入库+卜部白话精译）→ v2.7（4913条，白话精译大规模推进4895篇/99.6%）→ v2.8（4940条，钟吕传道集+麻衣神相入库+医部白话精译完成）→ v2.9（5018条，丹溪心法+太上感应篇+灵宝毕法+脾胃论+兰室秘藏+焦氏易林精选入库）→ v3.0（5251条，医部入门四典入库）→ v3.1（5773条，医部第二批五典入库）→ v3.2（5870条，医学衷中参西录+医学正传入库）→ v3.3（5893条，医学入门+本草纲目精选+景岳全书精选入库）→ v3.4（5926条，医方集解+本草备要+小儿药证直诀入库）→ v3.5（5929条，医宗金鉴精选+外台秘要精选入库）→ v3.6（6014条，针灸甲乙经12条扩展至97条）→ v3.7（6018条，山部符咒类4部入库）→ v3.8（6035条，太极拳5篇+北帝七元4部+易隐卷首8条入库）→ v3.9（6039条，易冒三篇序+道法会元精选入库）→ v4.0（6041条，道法会元卷四至卷十+盟威法箓入库）→ v4.1（6043条，易隐张星元总断+道法会元卷十一至卷二十入库）→ v4.2（6047条，道法会元卷二十一至卷三十+三天内解经+三要达道论入库）→ v4.3（6049条，道法会元卷三十一至卷三十六+虚靖真君语录入库）→ v4.4（6052条，道法会元卷三十七至卷四十六+三洞修炼科+上清修身经入库）→ v4.5（6055条，道法会元卷四十七至卷五十九+上清七圣玄纪经+上清丹景八术经入库）→ v4.6（6056条，高上神霄玉清真王紫书大法入库）→ v4.7（6057条，三洞修道仪入库）→ v4.8（6057条，太乙金镜式经入库，三式齐备）。
...
2. 古注 / 阐微与原文分层渲染，不得混排。
3. 【白话提要】只逐条文义串讲、**不新增任何现代断命、不编造**；《千里命稿》本身即白话，保留原讲解、不另译。当前全库4808条白话层已全部为针对性文义串讲（零模板占位），其中命部核心1547条为人工精译，其余3261条为基于原文首句+核心内容的提取式串讲。
...
- **Rust（生产）**：`engine/`，纯 `serde_json`、框架无关，Tauri 可直接依赖；
  ...
  `cd engine && cargo test`（含加载真实 4808 条 manifest 的用例）。
...
| 全量白话校补       | 4808 条全部针对性文义串讲（零模板）    | 命部核心1547条人工精译+其余3261条提取式串讲      | ✅  |
