---
id: "zpzq_ch23_liuqin"
book: "子平真诠评注"
chapter: "第23章·论宫分用神配六亲"
chapter_num: 23
section_title: "论宫分用神配六亲"
source_version: "沈孝瞻原著·徐乐吾评注(节本)"
author: "沈孝瞻"
dynasty: "清"
type: "chapter"
conditions:
  day_master: []
  month_branch: []
  day_pillar: []
  hour_pillar: []
  ten_god: []
  pattern: []
  shensha: []
  keywords: ["六亲", "宫分", "父母兄弟"]
weight: 8
tags: ["子平真诠", "论宫分用神配六亲"]
---

### 第23章 论宫分用神配六亲

**【原文】**

人有六亲，配之八字，亦存于命。

其由宫分配之者，则年月日时，自上而下，祖父妻子，亦自上而下。以地相配，适得其宜，不易之位也。

其由用神配之者，则正印也母，身所自出，取其生我也。若偏财受我克制，何反为父？偏财者，母之正夫也，正印为母，则偏财为父矣。正财为妻，受我克制，夫为妻纲，妻则从夫。若官煞则克制乎我，何以反为子女也？官煞者，财所生也，财为妻妾，则官煞为子女矣。至于比肩为兄弟，又理之显然者。

其间有无得力，或吉或凶，则以四柱所存或年月或日时财官伤刃，系是何物，然后以六亲配之用神。局中作何喜忌，参而配之，可以了然矣。

**【白话提要】**

本篇论六亲配命有两条线索。其一按宫分：年、月、日、时自上而下，依次配祖、父、妻（自身）、子，位置不易。其二按用神（十神）：生我者正印为母；偏财为母之正夫，故偏财为父；我克者正财为妻（妻从夫）；克我者官煞为财所生，故官煞为子女；同我者比肩为兄弟。至于各六亲得力与否、或吉或凶，则看四柱年月时中财官伤刃等是何物，再按格局喜忌配合，便可了然。
